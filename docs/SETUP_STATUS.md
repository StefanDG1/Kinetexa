...
6 September 2026. Milestone `0.1.0-alpha.2` prepares the project and accounts, including the live Stripe catalog. Full V1 implementation is the next phase.
...
| Stripe live    | Separate Kinetexa account, existing company details, Kinetexa website and descriptor; restricted key saved; EUR 35 monthly and EUR 180 annual prices; customer billing portal; production environment synchronized to Vercel | User verification completed. Live API catalog readback confirms amounts, currency and intervals. Active live portal allows payment-method updates, invoice history and cancellation at period end. Live payment/payout capability has not yet been independently confirmed |
...
The user completed Stripe's identity verification. The restricted live key was saved without exposing it in source control, and the live catalog and portal were created and read back through the API. The live key and billing identifiers are saved in Vercel's production environment for the next deployment; test resources remain in development.

No further user action blocks starting implementation. The restricted setup key cannot read Stripe account capability details (`GET /v1/account` returns a permissions error). The live dashboard is accessible, but successful catalog operations do not establish payment or payout readiness. Confirm those capabilities as part of the release checks. No real charge has been made.
