...
## Functional interface APIs, 0.5.0-alpha.1

`activities:browse` returns byte-bounded list/map/picker projections with pagination, dates, sport and text filters. `activities:selection` resolves one owned activity title from an untrusted link ID without exposing private metadata. `analytics:dashboard` includes backend goal progress and pinned query results; `analytics:goals` computes owned progress; `analytics:records` batches distance/power/pace curves in one historical read. These deterministic actions do not use AI tokens. `gear:convertLegacyReminder` atomically preserves a legacy reminder interval/baseline and removes the superseded fields, idempotently. Existing provenance paging, canonical downloads, interval calculations, gear service/reminder APIs and AI feedback now have functional controls.
