...
Snapshots are retained for seven days. Retention runs even if snapshot creation fails. Unreferenced backup objects older than one day are removed; objects needed by a retained snapshot remain. Temporary user export archives are excluded, and recovery marks their jobs expired. No private snapshot or object is uploaded as a public GitHub artifact.

Deletion first writes an independent tombstone to the backup bucket. A restore reads the current tombstones and filters the athlete plus every owned record and object, even if the selected snapshot predates deletion. Snapshots containing an account already marked for deletion also exclude it during recovery. Tombstones contain only the internal athlete ID and deletion time; they are retained independently of expiring snapshots. Business/privacy review of this retention schedule remains a release gate.
...
The initial isolated drill restored seven activities across two synthetic accounts, thirteen health records and 32 checksum-verified objects. Detailed current evidence is recorded in `verification-staging.md`.
