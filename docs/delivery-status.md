...
Latest backend milestone, `0.4.0-alpha.4`: 125 tests; server-calculated, privacy-limited share totals; exact expiry and per-link read limits; real-browser share/limit/revocation checks; bounded XML record parsing and canonical uploads; a hosted 28 MB GPX round trip; seven retained staging activities reprocessed without losing edits or history; and a clean restore after fixture removal. Production registration remains closed. [Evidence and limits](verification/backend-0.4.0-alpha.4.json).

Previous backend milestone, `0.4.0-alpha.3`: 119 focused tests, partial-archive retry and stale-worker protection, paginated workspace/calendar APIs, profile-timezone query grouping, accurate missing goal measurements and evidence, and private signed-webhook observations with provider-specific metrics. Hosted checks passed on staging and the closed production backend. The preceding clean restore verified 45 objects, seven streams, thirteen health records and deletion exclusion across 25 tables. [Machine-readable evidence](verification/backend-0.4.0-alpha.3.json) records exact check times. The approved COROS clarification was accepted by Resend; delivery and the provider response remain unverified. Remaining connector, operational and product gates are open.
...
Previous backend milestone, `0.4.0-alpha.2`: 113 focused tests, private KPI/AI feedback APIs, exact import completion timestamps, upload replay protection, full provenance paging, strict health calendar dates, bounded webhook body reads and corrected best-distance calculation. Hosted API checks passed in staging; signed webhook boundary checks and the closed registration gate passed in production. All seven retained staging activities were reprocessed with checksums, edits and previous calculations preserved. The replay fixture completed identity/object/application-data deletion and tombstone verification. Three temporary simulator deletion notices remain subject to the existing Free email quota and bounded retention. Full KPI observation, approved persistent connectors, remaining operational coverage, broader device/AI validation, frontend acceptance and business release gates remain open. External PostHog capture stays off pending re-authentication/key setup.
