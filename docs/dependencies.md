...
| Library                        | License       | Use                                                                 |
| ------------------------------ | ------------- | ------------------------------------------------------------------- |
| @internationalized/date 3.12.4 | Apache-2.0    | Profile-timezone dates and daylight-saving-safe calendar operations |
...
Adobe's `@internationalized/date` 3.12.4 metadata, repository and installed Apache-2.0 license were checked on 7 September 2026. Calendar inputs reject nonexistent local times; repeated times explicitly use the first occurrence. This avoids maintaining timezone transition rules in application code.
