...
`scripts/export-traces.mjs` converts the last hour of events into OTLP JSON envelopes in an ignored local file. Set `OPS_CONFIG` privately with `environment`, `convexUrl` and `deploymentKey`. Output creation never overwrites an existing file. Trace timing is marked unknown when a job did not record a start time. Activity imports and reprocessing now attach bounded storage-read/write, decode, parse/normalize and analytics phases to the existing outcome event. AI requests and automatic insights attach planner, tool and explanation phases. OTLP exports emit these as child spans with deterministic IDs. No extra database mutation is made per phase. Archive fan-out, watchdog termination and health-only paths can have incomplete phase coverage; separate normalization timing is not claimed when parsing performs it internally. Provider-to-ingestion and cross-service web traces remain open. No paid Vercel drain was enabled.
...
GitHub notification delivery depends on repository/account preferences and has not been verified. Missing webhook deliveries, activity-provider metrics, auth failure thresholds, browser vitals and full distributed tracing are outstanding acceptance items. Current monitoring does not complete OBS-001 through OBS-004.

## Collector delivery and phase alerts

Run `node scripts/export-traces.mjs work/traces.jsonl` for file export only. Add `--send` to also POST OTLP/HTTP JSON to the exact `otlpEndpoint` in private `OPS_CONFIG`, with optional `otlpHeaders`. Use HTTPS for remote collectors; plain HTTP is allowed only for loopback. Redirects and partial rejection fail the run. No automatic retry is performed; the local file retains the attempted envelopes. Re-exported spans have the same IDs but collector deduplication is implementation-dependent. No external collector has been configured or receipt-verified for this deployment.

The existing monitor aggregates phase event counts, failures and latency distributions without additional database reads. Five failed phases at or above 20% of a day's recorded phase attempts trigger `phase:<name>-failure-rate`, including storage read/write failures. These cover instrumented processing/AI paths, not all object-store or HTTP requests. Local tests verify timing, failure propagation, payload omission, parent/child IDs, aggregate latency and alert recovery. Hosted import/model tests were intentionally not repeated for this milestone.
