...
Empty denominators return `null`, never a made-up zero. Provider reliability, AI evaluation plus user-feedback quality and Open Solo adoption remain explicitly unavailable. Runtime model completion must not stand in for grounded-answer quality. Frontend event bindings, external retention configuration, external ingestion/erasure verification and full KPI acceptance remain open.
